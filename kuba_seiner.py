import sys
from PyQt5.QtWidgets import (QTableWidget, QTableWidgetItem, QWidget, QApplication, QInputDialog, QLineEdit,
                             QFileDialog, QHBoxLayout, QVBoxLayout, QLabel, QFileDialog, QTextEdit, QAction, qApp,
                             QDesktopWidget, QMainWindow, QWidget, QMessageBox, QToolTip, QPushButton)
...
class Detekce(QMainWindow):
    ...
    def vyber(self):
        ...
        try:
            p = np.reshape(U, (
            int(f), n))  # zařizuje přetvoření matice tak aby sloupec matice odpovídal sloupci vybranému z tabulky
            q = p.T
            print(q)
            print(np.shape(q))
            self.q = q
    ...
    def uprava(self):
        ...
        for j in range(0, (int(self.f)), 1):
            for i in range(5, n, 1):
                d = np.append(d, s[
                    i, j])  # je potřeba upravit ak aby se nebrali hodnoty ze všech sloupců ale udělali se nové matice pos loupcích
    ...
    def filterGNGD(self):
        self.uprava()
        print("kulový 1")
        print(self.d.shape, self.x.shape)
        try:
            f = pa.filters.FilterGNGD(n=int(self.f), mu=float(self.rl), w="zeros")
            print("kulový2")
            y, e, w = f.run(self.d, self.x)
